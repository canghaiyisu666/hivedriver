...
public class HiveQueryResultSet extends HiveBaseResultSet {
	...
	protected HiveQueryResultSet(Builder builder) throws SQLException {
		...
		if (builder.retrieveSchema) {
			...
			this.setSchema(builder.colNames, builder.colTypes,
					builder.colAttributes);
		}
	...
	private static JdbcColumnAttributes getColumnAttributes(
		...
		if (primitiveTypeEntry.isSetTypeQualifiers()) {
			...
			case CHAR_TYPE:
			case VARCHAR_TYPE:
				TTypeQualifierValue val = tq.getQualifiers().get(
				...
				break;
			case DECIMAL_TYPE:
				TTypeQualifierValue prec = tq.getQualifiers().get(
						TCLIServiceConstants.PRECISION);
				TTypeQualifierValue scale = tq.getQualifiers().get(
						TCLIServiceConstants.SCALE);
				ret = new JdbcColumnAttributes(
						prec == null ? HiveDecimal.USER_DEFAULT_PRECISION
								: prec.getI32Value(),
						scale == null ? HiveDecimal.USER_DEFAULT_SCALE : scale
								.getI32Value());
				break;
			default:
				break;
	...
	private void retrieveSchema() throws SQLException {
		try {
			...
			for (int pos = 0; pos < schema.getColumnsSize(); pos++) {
				...
				TPrimitiveTypeEntry primitiveTypeEntry = columns.get(pos)
						.getTypeDesc().getTypes().get(0).getPrimitiveEntry();
				String columnTypeName = TYPE_NAMES.get(primitiveTypeEntry
						.getType());
	...
	public boolean next() throws SQLException {
		...
		try {
			...
			if (fetchFirst) {
				// If we are asked to start from begining, clear the current
				// fetched resultset
