...
public class HiveConnection implements java.sql.Connection {
	public static final Log LOG = LogFactory.getLog(HiveConnection.class
			.getName());
	...
	private TCLIService.Iface client; // todo should be replaced by
										// CliServiceClient
	...
	public HiveConnection(String uri, Properties info) throws SQLException {
		...
		if (isEmbeddedMode) {
			...
			if (info.containsKey(HIVE_AUTH_USER)) {
				sessConfMap.put(HIVE_AUTH_USER,
						info.getProperty(HIVE_AUTH_USER));
				if (info.containsKey(HIVE_AUTH_PASSWD)) {
					sessConfMap.put(HIVE_AUTH_PASSWD,
							info.getProperty(HIVE_AUTH_PASSWD));
				}
			}
			if (info.containsKey(HIVE_AUTH_TYPE)) {
				sessConfMap.put(HIVE_AUTH_TYPE,
						info.getProperty(HIVE_AUTH_TYPE));
			}
	...
	private void openTransport() throws SQLException {
		// TODO: Refactor transport creation to a factory, it's getting uber
		// messy here
//		System.out.println(hiveConfMap.get("hive.metastore.thrift.framed.transport.enabled"));
		...
		try {
			...
			throw new SQLException("Could not open connection to " + jdbcURI
	...
	private String getServerHttpUrl(boolean useSsl) {
		...
		// JDBC driver will set up an https url if ssl is enabled, otherwise
		// http
	...
	private TTransport createHttpTransport() throws SQLException {
		...
		try {
			...
			String msg = "Could not create http connection to " + jdbcURI
					+ ". " + e.getMessage();
	...
	private DefaultHttpClient getHttpClient(Boolean useSsl) throws SQLException {
		...
		if (isKerberosAuthMode()) {
			...
			if (useSsl) {
				String sslTrustStorePath = sessConfMap
						.get(HIVE_SSL_TRUST_STORE);
				String sslTrustStorePassword = sessConfMap
						.get(HIVE_SSL_TRUST_STORE_PASSWORD);
				...
				try {
					if (sslTrustStorePath == null
						...
						sslTrustStore = KeyStore
								.getInstance(HIVE_SSL_TRUST_STORE_TYPE);
						sslTrustStore.load(new FileInputStream(
	...
	private TTransport createBinaryTransport() throws SQLException {
		try {
			...
			if (!HIVE_AUTH_SIMPLE.equals(sessConfMap.get(HIVE_AUTH_TYPE))) {
				...
				if (sessConfMap.containsKey(HIVE_AUTH_PRINCIPAL)) {
					if (sessConfMap.containsKey(HIVE_AUTH_QOP)) {
						try {
							saslQOP = SaslQOP.fromString(sessConfMap
									.get(HIVE_AUTH_QOP));
						} catch (IllegalArgumentException e) {
					...
					boolean assumeSubject = HIVE_AUTH_KERBEROS_AUTH_TYPE_FROM_SUBJECT
							.equals(sessConfMap
									.get(HIVE_AUTH_KERBEROS_AUTH_TYPE));
					transport = KerberosSaslHelper.getKerberosTransport(
							sessConfMap.get(HIVE_AUTH_PRINCIPAL), host,
							HiveAuthFactory.getSocketTransport(host, port,
									loginTimeout), saslProps, assumeSubject);
				} else {
					// If there's a delegation token available then use token
					// based connection
					String tokenStr = getClientDelegationToken(sessConfMap);
					if (tokenStr != null) {
						transport = KerberosSaslHelper.getTokenTransport(
								tokenStr, host, HiveAuthFactory
										.getSocketTransport(host, port,
												loginTimeout), saslProps);
					} else {
						...
						if (isSslConnection()) {
							// get SSL socket
							String sslTrustStore = sessConfMap
									.get(HIVE_SSL_TRUST_STORE);
							String sslTrustStorePassword = sessConfMap
									.get(HIVE_SSL_TRUST_STORE_PASSWORD);
							if (sslTrustStore == null
									|| sslTrustStore.isEmpty()) {
								transport = HiveAuthFactory.getSSLSocket(host,
										port, loginTimeout);
							} else {
								transport = HiveAuthFactory.getSSLSocket(host,
										port, loginTimeout, sslTrustStore,
										sslTrustStorePassword);
							}
						} else {
							// get non-SSL socket transport
							transport = HiveAuthFactory.getSocketTransport(
									host, port, loginTimeout);
						}
						// Overlay the SASL transport on top of the base socket
						// transport (SSL or non-SSL)
						transport = PlainSaslHelper.getPlainTransport(userName,
								passwd, transport);
					}
				}
			} else {
			...
			throw new SQLException("Could not create connection to " + jdbcURI
					+ ": " + e.getMessage(), " 08S01", e);
		}
		return transport;
	}
	...
	private String getClientDelegationToken(Map<String, String> jdbcConnConf)
		...
		if (HIVE_AUTH_TOKEN.equalsIgnoreCase(jdbcConnConf.get(HIVE_AUTH_TYPE))) {
			...
			try {
				tokenStr = ShimLoader.getHadoopShims().getTokenStrForm(
	...
	private void openSession(Utils.JdbcConnectionParams connParams)
		...
		for (Entry<String, String> hiveConf : connParams.getHiveConfs()
				.entrySet()) {
			openConf.put("set:hiveconf:" + hiveConf.getKey(),
					hiveConf.getValue());
		}
		// For remote JDBC client, try to set the hive var using 'set
		// hivevar:key=value'
		for (Entry<String, String> hiveVar : connParams.getHiveVars()
				.entrySet()) {
		...
		// Store the user name in the open request in case no non-sasl
		// authentication
		if (HIVE_AUTH_SIMPLE.equals(sessConfMap.get(HIVE_AUTH_TYPE))) {
		...
		try {
			...
			if (!supportedProtocols.contains(openResp
	...
	private void setupLoginTimeout() {
		long timeOut = TimeUnit.SECONDS.toMillis(DriverManager
				.getLoginTimeout());
	...
	public String getDelegationToken(String owner, String renewer)
			throws SQLException {
		TGetDelegationTokenReq req = new TGetDelegationTokenReq(sessHandle,
				owner, renewer);
		try {
	...
	public void cancelDelegationToken(String tokenStr) throws SQLException {
		TCancelDelegationTokenReq cancelReq = new TCancelDelegationTokenReq(
				sessHandle, tokenStr);
		try {
			TCancelDelegationTokenResp cancelResp = client
					.CancelDelegationToken(cancelReq);
			Utils.verifySuccess(cancelResp.getStatus());
			return;
		} catch (TException e) {
			throw new SQLException("Could not cancel token: " + e.getMessage(),
					" 08S01", e);
		}
	}
	...
	public void renewDelegationToken(String tokenStr) throws SQLException {
		TRenewDelegationTokenReq cancelReq = new TRenewDelegationTokenReq(
				sessHandle, tokenStr);
		try {
			TRenewDelegationTokenResp renewResp = client
					.RenewDelegationToken(cancelReq);
			Utils.verifySuccess(renewResp.getStatus());
			return;
		} catch (TException e) {
			throw new SQLException("Could not renew token: " + e.getMessage(),
					" 08S01", e);
		}
	}
	...
	public Statement createStatement() throws SQLException {
		if (isClosed) {
			throw new SQLException(
					"Can't create Statement, connection is closed");
		}
	...
	public Statement createStatement(int resultSetType, int resultSetConcurrency)
		...
		if (resultSetType == ResultSet.TYPE_SCROLL_SENSITIVE) {
			throw new SQLException("Statement with resultset type "
					+ resultSetType + " is not supported", "HYC00"); // Optional
																		// feature
																		// not
																		// implemented
		}
